 ...
 * @author Freedman
 *
 * A household object for MORPC
 */
//public class Household implements java.io.Serializable {
public class Household implements java.io.Externalizable {
	...
	public float getPatternTypeAlt (int alternative) {
		switch (alternative) {
			...
			case 8:
				...
				break;
			case 9:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 1)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][0]].getPatternType();
				break;
			case 10:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 2)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][1]].getPatternType();
				break;
			case 11:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 3)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][2]].getPatternType();
				break;
			case 12:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 4)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][3]].getPatternType();
				break;
			case 13:
	...
	public float getJointToursPersonAlt (int alternative) {
		switch (alternative) {
			...
			case 9:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 1)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][0]].getJointTourCount();
				break;
			case 10:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 2)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][1]].getJointTourCount();
				break;
			case 11:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 3)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][2]].getJointTourCount();
				break;
			case 12:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 4)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][3]].getJointTourCount();
				break;
			case 13:
				if (personsByPersonTypeArray[PersonType.NONWORKER].length >= 1)
	...
	public float getTimeWindowAvailAlt (int alternative) {
		switch (alternative) {
			...
			case 9:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 1)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][0]].getAvailableWindow();
				break;
			case 10:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 2)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][1]].getAvailableWindow();
				break;
			case 11:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 3)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][2]].getAvailableWindow();
				break;
			case 12:
				if (personsByPersonTypeArray[PersonType.STUDENT].length >= 4)
					return persons[personsByPersonTypeArray[PersonType.STUDENT][3]].getAvailableWindow();
				break;
			case 13:
				if (personsByPersonTypeArray[PersonType.NONWORKER].length >= 1)
		...
		for (int i=1; i < persons.length; i++) {
			if (persons[i].getPatternType() == PatternType.HOME &&
				(persons[i].getPersonType() == PersonType.NONWORKER ||
					...
					persons[i].getPersonType() == PersonType.STUDENT))
						return 1;
		}
		return 0;
	}
	...
	/**
	 * return 1 if a student stays home; 0 otherwise.
	 */
	public float getStudentHome () {
		for (int i=1; i < persons.length; i++) {
			if (persons[i].getPatternType() == PatternType.HOME &&
				persons[i].getPersonType() == PersonType.STUDENT )
					return 1;
	...
	public float getPersonNonMandatoryMaint () {
		...
		int count = getNumberOfNonMandatoryParticipationsByType ( TourType.ESCORTING )
	...
	public float getWorkStudNonMandatoryTours () {
		for (int p=1; p < persons.length; p++) {
			if (persons[p].getPatternType() == PatternType.NON_MAND &&
				( persons[p].getPersonType() == PersonType.STUDENT || persons[p].getPersonType() == PersonType.WORKER_F || persons[p].getPersonType() == PersonType.WORKER_P ) )
					return 1;
		}
		return 0;
	}
	...
	/**
	 * return 1 if at least one university student participates in the joint tour
	 */
	public float getUnivInJointTour () {
		return ( jointTours[tourID].getNumUniv() ) > 0 ? 1 : 0;
	}
	...
	/**
	 * get the number of student persons in the Household.
	 */
	public int getStudPersons () {
		return personsByType[PersonType.STUDENT];
	}
	...
	*/
	...
	/**
	 * return the size for the given ob stop location choice alternative for university purpose
	*/
	public float getSizeStopUnivOBAlt (int alt) {
		return getSizeStopAlt (0, TourType.UNIVERSITY, alt);
	}
	...
	*/
	...
	/**
	 * return the size for the given ib stop location choice alternative for university purpose
	*/
	public float getSizeStopUnivIBAlt (int alt) {
		return getSizeStopAlt (1, TourType.UNIVERSITY, alt);
	}
	...
	*/
	...
	/**
	 * return the county for the given destination choice alternative
	*/
	public float getCountyDestAlt (int alt) {
		return ZonalDataManager.cnty[alt];
	}

	/**
	 * return the county for this tour's origin
	*/
	public float getCountyOrig () {
	    int alt = origTaz*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.cnty[alt];
	}

	/**
	 * return the county for this tour's destination
	*/
	public float getCountyDest () {
		int alt = chosenDest*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.cnty[alt];
	}

	/**
	 * return the school district for the given destination choice alternative
	*/
	public float getSchdistDestAlt (int alt) {
		return ZonalDataManager.schdist[alt];
	}

	/**
	 * return the school district for this tour's origin
	*/
	public float getSchdistOrig () {
		int alt = origTaz*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.schdist[alt];
	}

	/**
	 * return the school district for this tour's destination
	*/
	public float getSchdistDest () {
		int alt = chosenDest*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.schdist[alt];
	}

	/**
	 * return the ring district for the given destination choice alternative
	*/
	public float getRingAlt (int alt) {
		return ZonalDataManager.rings[alt];
	}

	/**
	 * return the ring district for this tour's origin
	*/
	public float getRingOrig () {
		int alt = origTaz*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.rings[alt];
	}

	/**
	 * return the ring district for this tour's destination
	*/
	public float getRingDest () {
		int alt = chosenDest*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.rings[alt];
	}

	/**
	 * return the super district for the given destination choice alternative
	*/
	public float getSuprdistAlt (int alt) {
		return ZonalDataManager.suprdist[alt];
	}

	/**
	 * return the super district for this tour's origin
	*/
	public float getSuprdistOrig () {
		int alt = origTaz*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.suprdist[alt];
	}

	/**
	 * return the super district for this tour's destination
	*/
	public float getSuprdistDest () {
		int alt = chosenDest*ZonalDataManager.WALK_SEGMENTS;
		return ZonalDataManager.suprdist[alt];
	}
	...
	*/
	...
	/**
	 * return the chosen alternative from the free parking eligible model
	*/
	public float getFreeParking () {
		return persons[personID].getFreeParking();
	}
	...
	*/
	...
	/**
	 * return the urban type for the chosen destination choice alternative
	*/
	...
	/**
	 * return the urban type for the tour origin
	*/
	...
	/**
	 * return true or false that the destination taz is in the cbd.
	*/
	public boolean getCbdDest () {
		return ZonalDataManager.zonalAreaType[chosenDest] == 1;
	}

	/**
	 * return true or false that the destination taz is in the cbd.
	*/
	public float getCbdAreaTypeDest () {
		return ZonalDataManager.zonalAreaType[chosenDest];
	}

	/**
	 * return the cbd areatype for the given parking location choice alternative
	*/
	public float getCbdAreaTypeStopAlt (int alt) {
		return ZonalDataManager.cbdatype[alt];
	}

	/**
	 * return the total parking spaces for the given parking location choice alternative
	*/
	public float getParkTotStopAlt (int alt) {
		return ZonalDataManager.parktot[alt];
	}

	/**
	 * return the total long term parking spaces for the given parking location choice alternative
	*/
	public float getParkLongStopAlt (int alt) {
		return ZonalDataManager.parklng[alt];
	}

	/**
	 * return the parking rate for the given parking location choice alternative
	*/
	public float getParkRateStopAlt (int alt) {
		return ZonalDataManager.parkrate[alt];
	}

	/**
	 * return the proportion free parking for the given parking location choice alternative
	*/
	public float getPropFreeStopAlt (int alt) {
	...
	*/
	public float getZonal_nonw_walk_orig () {
	...
	*/
	public float getZonalShortWalkAccessDest () {
		int alt = (chosenDest-1)*ZonalDataManager.WALK_SEGMENTS + chosenWalkSegment;
		return ZonalDataManager.zonalShortAccess[alt];
	}

	/**
	 * return the parking rate for the chosen destination alt for this tour
	*/
	public float getParkRateDest () {
		return ZonalDataManager.parkRate[chosenDest];
	}
