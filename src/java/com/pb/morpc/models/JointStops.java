...
public class JointStops implements java.io.Serializable {
    ...
    public JointStops (HashMap<String,String> propertyMap, HouseholdArrayManager hhMgr) {
        ...
        if ( numThreads <= 0 ) {
            ...
            stopsHH[0] = new StopsHousehold ( propertyMap, TourType.JOINT_CATEGORY, TourType.JOINT_TYPES );
    ...
    public void doSfcSlcWork() {
        ...
        if ( numThreads <= 0 ) {
            for (int i=0; i < hhList.length; i++) {
                ...
                stopsHH[0].jointTourSfcSlc (hhList[i]);
            }
        }
        else {
    ...
    public void doSmcWork() {
        ...
        if ( numThreads <= 0 ) {
            for (int i=0; i < hhList.length; i++) {
                hhList[i].setProcessorIndex ( 0 );       
                stopsHH[0].jointTourSmc (hhList[i]);
            }
        }
        else {
    ...
    private void calculateSfcSlcDistributed( HashMap<String,String> propertyMap, Household[] hhList, int numThreads ) {
        ...
        for (int[] startEndIndices : startEndIndexList)
        {
            ...
            logger.info(String.format("creating JointSfcSlcTASK: %d range: %d to %d.", taskIndex, startIndex, endIndex));

            if( stopsHH[taskIndex] == null )
                stopsHH[taskIndex] = new StopsHousehold ( taskIndex, propertyMap, TourType.JOINT_CATEGORY, TourType.JOINT_TYPES );

            JointSfcSlcTask task = new JointSfcSlcTask( stopsHH[taskIndex], taskIndex, startIndex, endIndex, hhList );
        ...
        for (Future<List<Object>> fs : results) {
            ...
            try {
                ...
                logger.info(String.format("returned JointSfcSlcTask %d of %d, start=%d, end=%d.", task, numPackets, start, end));
            }
    ...
    private void calculateSmcDistributed( HashMap<String,String> propertyMap, Household[] hhList, int numThreads ) {
        ...
        for (int[] startEndIndices : startEndIndexList)
        {
            ...
            logger.info(String.format("creating JointSmcTASK: %d range: %d to %d.", taskIndex, startIndex, endIndex));

            JointSmcTask task = new JointSmcTask( stopsHH[taskIndex], taskIndex, startIndex, endIndex, hhList );
        ...
        for (Future<List<Object>> fs : results) {
            ...
            try {
                ...
                int start = (Integer) resultBundle.get(1);
                int end = (Integer) resultBundle.get(2);
                logger.info(String.format("returned JointSmcTASK %d of %d, start=%d, end=%d.", task, numPackets, start, end));
            }
            catch (InterruptedException e) {
                logger.error("InterruptedException returned in place of result object.", e);
                System.exit(-1);
