...
public class AtWorkDTM implements java.io.Serializable {
    ...
    public AtWorkDTM (HashMap<String,String> propertyMap, HouseholdArrayManager hhMgr) {
        ...
        if ( numThreads <= 0 ) {
            ...
            dtmHH[0] = new DTMHousehold ( propertyMap, TourType.AT_WORK_CATEGORY, TourType.AT_WORK_TYPES );
    ...
    public void doWork() {
        ...
        if ( numThreads <= 0 ) {
            ...
            for (int i=0; i < hhList.length; i++) {
                ...
                dtmHH[0].atWorkTourDc (hhList[i]);
                dtmHH[0].atWorkTourTc (hhList[i]);
                dtmHH[0].atWorkTourMc (hhList[i]);
            }
        }
        else {
    ...
    private void calculateDistributed( HashMap<String,String> propertyMap, Household[] hhList, int numThreads ) {
        ...
        for (int[] startEndIndices : startEndIndexList)
        {
            ...
            logger.info(String.format("creating AtWork DcTcMcTASK: %d range: %d to %d.", taskIndex, startIndex, endIndex));

            dtmHH[taskIndex] = new DTMHousehold ( taskIndex, propertyMap, TourType.AT_WORK_CATEGORY, TourType.AT_WORK_TYPES );

            AtWorkDcTcMcTask task = new AtWorkDcTcMcTask( dtmHH[taskIndex], taskIndex, startIndex, endIndex, hhList );
        ...
        for (Future<List<Object>> fs : results) {
            ...
            try {
                ...
                logger.info(String.format("returned atWork dcTcMcTask %d of %d, start=%d, end=%d.", task, numPackets, start, end));
            }
