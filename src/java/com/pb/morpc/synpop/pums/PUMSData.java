...
public class PUMSData {
    ...
    protected static Logger logger = Logger.getLogger("com.pb.morpc.synpop.pums");
    ...
    **/
    public int getAttribIndex (String Variable) {
        ...
        if (index < 0) {
            logger.severe("variable " + Variable + " not defined in list of attributes for pumsHH[0]");
	...
	public void readData (String fileName) {
			...
			while ((s = in.readLine()) != null) {
				...
				if ((getPUMSHHDataValue (s, hh.attribLabels[personsField])) > 0) {
					...
					if (getPUMSRecType(s).equals("H")) {
						...
						for (int i=0; i < hh.attribs.length; i++) {
							hh.attribs[i] = (getPUMSHHDataValue (s, hh.attribLabels[i]));
						}
						...
						for (int i=0; i < hh.attribs[personsField]; i++) {
							...
							school = getPUMSPersDataValue (s, "SCHOOL");
					...
					for (int i=0; i < hh.attribs.length; i++) {
						if (hh.attribLabels[i].equals("RWRKR89")) {
		...
		logger.info ("person field value different from number of person records in " + invalid + " of " + hhCount + " households.");
	}
...
// the following main() is used to test the methods implemented in this object.
    public static void main (String[] args) {
        ...
        String PUMSFILE = "c:\\jim\\morpc\\data\\pums\\pumsaxor.txt";
        String PUMSDICT = "c:\\jim\\morpc\\data\\pums\\pumsusdd.txt";
...
//        pums.printPUMSDictionary ();
		...
		logger.info (pums.getPUMShhCount() + " households read from " + pums.getPUMSRecCount() + " PUMS records.");
    }
}
