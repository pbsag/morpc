 ...
 * @author Jim Hicks
 *
 * worker class for running joint stop frequency and stop location choice models for
...
public class JointStopsWorker extends MessageProcessingTask implements java.io.Serializable {
		...
		if ( msg.getSender().equals( modelServer ) ) {
			...
			if ( msg.getId().equals( MessageID.HOUSEHOLD_LIST )	) {
				...
				if ( hhList != null ) {
					...
					if (LOGGING)
						logger.info ( this.getName() + " processing joint stop freq/loc household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					for (int i=0; i < hhList.length; i++) {
						try {
						    ...
						    e.printStackTrace();
							logger.severe ("runtime exception occurred in joint stop freq/loc for household id=" + hhList[i].getID() + "in " + this.getName() );
					...
					if (LOGGING)
					    logger.info ( this.getName() + " processing joint stop mode household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					for (int i=0; i < hhList.length; i++) {
						try {
							...
							e.printStackTrace();
							logger.severe ("runtime exception occurred in joint stop mode for household id=" + hhList[i].getID() + "in " + this.getName() );
							logger.severe(e.getMessage());
							hhList[i].writeContentToLogger(logger);
							System.exit(-1);
						}
					...
					stopsHH.printTimes ( TourType.JOINT_CATEGORY );
