 ...
 * @author Jim Hicks
 *
 * worker class for running at-work stop frequency and stop location choice models for
...
public class AtWorkStopsWorker extends MessageProcessingTask implements java.io.Serializable {
	...
	private int respondToMessage (Message msg) {
		...
		if ( msg.getSender().equals( modelServer ) ) {
			...
			if (msg.getId().equals(MessageID.START_INFO)) {
				...
				if (LOGGING)
				    logger.info (this.getName() + " stopsHH object built for at-work stops, asking for work.");
			...
			if ( msg.getId().equals( MessageID.HOUSEHOLD_LIST )	) {
				...
				if ( hhList != null ) {
					...
					// run the stops models for the at-work tours in these households
					if (LOGGING)
						logger.info ( this.getName() + " processing at-work stop freq/loc household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					...
					if (LOGGING)
					    logger.info ( this.getName() + " processing at-work stop mode household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					...
					stopsHH.printTimes ( TourType.AT_WORK_CATEGORY );
