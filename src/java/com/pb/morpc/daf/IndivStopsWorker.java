	...
	private int respondToMessage (Message msg) {
		...
		if ( msg.getSender().equals( modelServer ) ) {
			...
			if (msg.getId().equals(MessageID.START_INFO)) {
				...
				if (LOGGING)
				    logger.info (this.getName() + " building stopsHH object for individual non-mandatory sfc, slc, and smc.");
				if (stopsHH == null)
    				stopsHH = new StopsHousehold ( processorId, propertyMap, TourType.NON_MANDATORY_CATEGORY, TourType.NON_MANDATORY_TYPES );
				...
				if ( hhList != null ) {
					...
					for (int i=0; i < hhList.length; i++) {
						try {
						    ...
						    e.printStackTrace();
							logger.severe ("runtime exception occurred in indiv non-mandatory stop freq/loc for household id=" + hhList[i].getID() + "in " + this.getName() );
					...
					for (int i=0; i < hhList.length; i++) {
						try {
							...
							e.printStackTrace();
							logger.severe ("runtime exception occurred in indiv non-mandatory stop mode choice for household id=" + hhList[i].getID() + "in " + this.getName() );
							logger.severe(e.getMessage());
							hhList[i].writeContentToLogger(logger);
							System.exit(-1);
						}
