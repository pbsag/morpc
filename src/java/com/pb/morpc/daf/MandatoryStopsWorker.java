 ...
 * @author Jim Hicks
 *
 * worker class for running mandatory stop frequency and stop location choice models for
...
public class MandatoryStopsWorker extends MessageProcessingTask implements java.io.Serializable {
		...
		if ( msg.getSender().equals( modelServer ) ) {
			...
			if ( msg.getId().equals( MessageID.HOUSEHOLD_LIST )	) {
				...
				if ( hhList != null ) {
					...
					// run the stops models for the mandatory tours in these households
					if (LOGGING)
						logger.info ( this.getName() + " processing mandatory stop freq/loc household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					for (int i=0; i < hhList.length; i++) {
						try {
						    ...
						    e.printStackTrace();
							logger.severe ("runtime exception occurred in mandatory stop freq/loc for household id=" + hhList[i].getID() + "in " + this.getName() );
					...
					if (LOGGING)
					    logger.info ( this.getName() + " processing mandatory stop mode household ids: " + hhList[0].getID() + " to " + hhList[hhList.length-1].getID() );
					for (int i=0; i < hhList.length; i++) {
						try {
							...
							e.printStackTrace();
							logger.severe ("runtime exception occurred in mandatory stop mode for household id=" + hhList[i].getID() + "in " + this.getName() );
							logger.severe(e.getMessage());
							hhList[i].writeContentToLogger(logger);
							System.exit(-1);
						}
					...
					stopsHH.printTimes ( TourType.MANDATORY_CATEGORY );
