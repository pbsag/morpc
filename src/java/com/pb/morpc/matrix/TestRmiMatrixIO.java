...
//    static final boolean USE_RMI = false;
    ...
    private void runTest( String[] fileNameArgs ) {
        ...
        String tempMatrixName = null;
        
        if ( fileNameArgs.length >= 1 )
            inputMatrixFileName = fileNameArgs[0];
        else
            inputMatrixFileName = INPUT_MATRIX;
        tempMatrixName = inputMatrixFileName + "_revised";
        ...
        MatrixWriter mw = MatrixWriter.createWriter(MatrixType.TPPLUS, new File(tempMatrixName));
        ...
        mr = MatrixReader.createReader(MatrixType.TPPLUS, new File(tempMatrixName));
        System.out.println ( "created an object of " + mr.getClass().getName() + " for reading matrix data." );
